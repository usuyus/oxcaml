...
(* This file contains various tests for [int#].  It's not an expect test
...
let test1 () =
  (* CR layouts: When untagged_immediate defs are allowed at the module level, get rid of
  ...
  print_intu_bin "Test1, lognot_three (~0b00000011)" lognot_three;

  let three_shl_eight = shift_left three 8 in
  print_intu_bin "Test1, three_shl_eight (0b00000011 << 8)" three_shl_eight;

  let three_shr_one = shift_right three 1 in
  print_intu_bin "Test1, three_shr_one (0b00000011 >> 1)" three_shr_one;

  let three_shrl_one = shift_right_logical three 1 in
  print_intu_bin "Test1, three_shr_one (0b00000011 >>> 1)" three_shrl_one;
  ...
  print_intu_bin "Test1, lognot_minus_five (~0b1...1011)" lognot_minus_five;

  let minus_five_shl_eight = shift_left minus_five 8 in
  print_intu_bin "Test1, minus_five_shl_eight (0b1...1011 << 8)" minus_five_shl_eight;

  let minus_five_shr_one = shift_right minus_five 1 in
  print_intu_bin "Test1, minus_five_shr_one (0b1...1011 >> 1)" minus_five_shr_one;

  let minus_five_shrl_one = shift_right_logical minus_five 1 in
  print_intu_bin "Test1, minus_five_shr_one (0b1...1011 >>> 1)" minus_five_shrl_one;
  ...
  (* print_intu "Test 1, max_int" max_int;
   * print_intu "Test 1, min_int" min_int; *)
...
(* CR layouts v1.5: This type definition can be eliminated once we have
   annotations. *)
type ('a : untagged_immediate) t_untagged_immediate = 'a

let[@inline never] twice f (x : 'a t_untagged_immediate) = f (f x)
...
(* [go]'s closure should haave an [int] (immediate), a [int#] (untagged_immediate) and a
   [int array] (value). *)
...
(* many args - even args are tuples, odd args are unboxed ints *)
let[@inline_never] f3_manyargs x0 x1 x2 x3 x4 x5 x6 x7 x8 x9 steps () =
  ...
  let[@inline never] rec go k =
    ...
    then (Int_u.of_int 0)
      ...
      let acc = go (k + 1) in
      steps.(k) <- Int_u.to_int acc;
      Int_u.(acc + ((x1 + x3 + x5 + x7 + x9) * (of_int ((fun x -> x) sum))))
    end
  in
  go start_k
...
let test3 () =
  ...
  *)
  ...
  print_intu "Test 3, 171: " (f3_manyargs ());
  Array.iteri (Printf.printf "  Test 3, step %d: %d\n") steps
...
let[@inline never] test4 () =
  ...
  print_intu "Test 4, 1 - 2" (Int_u.of_int x2);
...
let[@inline never] f5 n m =
  let open Int_u in
  (* Also testing a closure with only int# values *)
  let[@inline never] go f =
    f (n + m)
  in
  go
...
(*********************************)
(* Test 6: methods on ints *)

(* CR layouts: add tests that capture ints in objects, once that is
   allowed. *)

(* int# args and returns *)
let f6_1 () = object
...
(* overapplication to int# and non-int# args *)
let f6_3 n k = object
...
(*********************************************)
(* Test 7: ints and assert false joins *)
